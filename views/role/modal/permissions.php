            ...
            <label for="action-<?php echo $action->id; ?>" class="list-group-item form-check form-switch">
                <div class="d-flex justify-content-start align-items-center">
                    <div class="form-check form-switch">
                        <?php echo Html::checkbox('checkbox', false, [
                            'id' => "action-{$action->id}",
                    ...
                    <div>
                        <p class="m-0"><?php echo $action->action; ?></p>
                        <p class="m-0 text-muted"><?php echo $action->action; ?></p>
                    </div>
                </div>
            </label>
        </div>
...
<script>
    function actionSwitch(input, event) {
        ...
        $.ajax({
            url: BASEURL + '/' + LANGUAGE + '/system/role/save-permission',
            method: 'POST',
            data: JSON.stringify({
                action_id: actionId,
